...
void PFAlgo3::runPF(Region &r) const {
  ...
  if (debug_) {
    dbgPrintf(
        ...
        r.phiCenter + r.phiHalfWidth);
    dbgPrintf("%s","PFAlgo3 \t N(track) %3lu   N(em) %3lu   N(calo) %3lu   N(mu) %3lu\n",
              r.track.size(),
              r.emcalo.size(),
              r.calo.size(),
              r.muon.size());
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
      ...
      dbgPrintf(
          "%s","PFAlgo3 \t track %3d: pt %7.2f +- %5.2f  vtx eta %+5.2f  vtx phi %+5.2f  calo eta %+5.2f  calo phi %+5.2f  "
    ...
    for (int iem = 0, nem = r.emcalo.size(); iem < nem; ++iem) {
      ...
      dbgPrintf(
          "%s","PFAlgo3 \t EM    %3d: pt %7.2f +- %5.2f  vtx eta %+5.2f  vtx phi %+5.2f  calo eta %+5.2f  calo phi %+5.2f  "
    ...
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
      auto &calo = r.calo[ic];
      dbgPrintf(
          "%s","PFAlgo3 \t calo  %3d: pt %7.2f +- %5.2f  vtx eta %+5.2f  vtx phi %+5.2f  calo eta %+5.2f  calo phi %+5.2f  "
    ...
    for (int im = 0, nm = r.muon.size(); im < nm; ++im) {
      auto &mu = r.muon[im];
      dbgPrintf(
          "%s","PFAlgo3 \t muon  %3d: pt %7.2f           vtx eta %+5.2f  vtx phi %+5.2f  calo eta %+5.2f  calo phi %+5.2f  "
          "fid %1d \n",
          im,
          mu.floatPt(),
          mu.floatEta(),
...
void PFAlgo3::link_tk2mu(Region &r, std::vector<int> &tk2mu, std::vector<int> &mu2tk) const {
  ...
  for (int imu = 0, nmu = r.muon.size(); imu < nmu; ++imu) {
    ...
    if (debug_)
      dbgPrintf(
          "%s","PFAlgo3 \t muon  %3d (pt %7.2f, eta %+5.2f, phi %+5.2f) \n", imu, mu.floatPt(), mu.floatEta(), mu.floatPhi());
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
      ...
      if (debug_ && dr < 0.4) {
        dbgPrintf(
            "%s","PFAlgo3 \t\t possible match with track %3d (pt %7.2f, caloeta %+5.2f, calophi %+5.2f, dr %.2f, eta "
    ...
    if (debug_ && imatch > -1)
      dbgPrintf("%s","PFAlgo3 \t muon  %3d (pt %7.2f) linked to track %3d (pt %7.2f)\n",
                imu,
                mu.floatPt(),
                imatch,
                r.track[imatch].floatPt());
    if (debug_ && imatch == -1)
      dbgPrintf("%s","PFAlgo3 \t muon  %3d (pt %7.2f) not linked to any track\n", imu, mu.floatPt());
    mu2tk[imu] = imatch;
    if (imatch > -1) {
      tk2mu[imatch] = imu;
...
void PFAlgo3::link_tk2em(Region &r, std::vector<int> &tk2em) const {
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (debug_ && tk2em[itk] != -1)
      dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) matches to EM   %3d (pt %7.2f) with dr %.3f\n",
                itk,
                tk.floatPt(),
                tk2em[itk],
...
void PFAlgo3::link_em2calo(Region &r, std::vector<int> &em2calo) const {
  ...
  for (int iem = 0, nem = r.emcalo.size(); iem < nem; ++iem) {
    ...
    if (debug_ && em2calo[iem] != -1)
      dbgPrintf("%s","PFAlgo3 \t EM    %3d (pt %7.2f) matches to calo %3d (pt %7.2f, empt %7.2f) with dr %.3f\n",
                iem,
                em.floatPt(),
                em2calo[iem],
...
void PFAlgo3::emcalo_algo(Region &r,
  ...
  for (int iem = 0, nem = r.emcalo.size(); iem < nem; ++iem) {
    ...
    if (r.globalAbsEta(em.floatEta()) > 2.5)
      continue;
    if (debug_)
      dbgPrintf("%s","PFAlgo3 \t EM    %3d (pt %7.2f) has %2d tracks (sumpt %7.2f, sumpterr %7.2f), ptdif %7.2f +- %7.2f\n",
                iem,
                em.floatPt(),
                em2ntk[iem],
                em2sumtkpt[iem],
    ...
    if (em2ntk[iem] == 0) {  // Photon
      ...
      em.used = true;
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t EM    %3d (pt %7.2f)    ---> promoted to photon\n", iem, em.floatPt());
      continue;
    }
    ...
    if (pterr > 2 * em.floatPt()) {
      pterr = 2 * em.floatPt();
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t EM    %3d (pt %7.2f)    ---> clamp pterr ---> new ptdiff %7.2f +- %7.2f\n",
                  iem,
                  em.floatPt(),
                  ptdiff,
                  pterr);
    ...
    if (ptdiff > -ptMatchLow_ * pterr) {
      ...
      if (ptdiff > +ptMatchHigh_ * pterr) {
        ...
        p.setFloatPt(ptdiff);
        if (debug_)
          dbgPrintf("%s","PFAlgo3 \t EM    %3d (pt %7.2f)    ---> promoted to electron(s) + photon (pt %7.2f)\n",
                    iem,
                    em.floatPt(),
                    ptdiff);
      } else {
        ...
        if (debug_)
          dbgPrintf("%s","PFAlgo3 \t EM    %3d (pt %7.2f)    ---> promoted to electron(s)\n", iem, em.floatPt());
      }
    } else {
      em.isEM = false;
      em.used = false;
...
void PFAlgo3::emtk_algo(Region &r,
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (em.isEM) {
      ...
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) matched to EM   %3d (pt %7.2f) promoted to electron with pt %7.2f\n",
                  itk,
                  tk.floatPt(),
                  tk2em[itk],
                  em.floatPt(),
...
void PFAlgo3::sub_em2calo(Region &r, const std::vector<int> &em2calo) const {
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    ...
    for (int iem = 0, nem = r.emcalo.size(); iem < nem; ++iem) {
      if (em2calo[iem] == ic) {
        ...
        if (em.isEM) {
          if (debug_)
            dbgPrintf(
                "%s","PFAlgo3 \t EM    %3d (pt %7.2f) is  subtracted from calo %3d (pt %7.2f) scaled by %.3f (deltaPt = "
                "%7.2f)\n",
                iem,
                em.floatPt(),
                ic,
          ...
          if (debug_)
            dbgPrintf(
                "%s","PFAlgo3 \t EM    %3d (pt %7.2f) not subtracted from calo %3d (pt %7.2f), and calo marked to be kept "
    ...
    if (pt < pt0) {
      if (debug_)
        dbgPrintf(
            "%s","PFAlgo3 \t calo  %3d (pt %7.2f +- %7.2f) has a subtracted pt of %7.2f, empt %7.2f -> %7.2f, isem %d\n",
            ic,
            calo.floatPt(),
            calo.floatPtErr(),
            pt,
      ...
      if (!keepme &&
        ...
        if (debug_)
          dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f)    ----> discarded\n", ic, calo.floatPt());
        calo.used = true;
...
void PFAlgo3::link_tk2calo(Region &r, std::vector<int> &tk2calo) const {
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (debug_)
      dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) to be matched to calo, min pT %7.2f\n", itk, tk.floatPt(), minCaloPt);
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
      auto &calo = r.calo[ic];
      ...
      switch (tkCaloLinkMetric_) {
        ...
        case TkCaloLinkMetric::BestByDRPt:
          ...
          //if (debug_ && dr < 0.2) dbgPrintf("%s","PFAlgo3 \t\t\t track %3d (pt %7.2f) vs calo %3d (pt %7.2f): dr %.3f, dq %.3f\n", itk, tk.floatPt(), ic, calo.floatPt(), dr, dq);
    ...
    if (debug_ && tk2calo[itk] != -1)
      dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) matches to calo %3d (pt %7.2f) with dist %.3f\n",
                itk,
                tk.floatPt(),
                tk2calo[itk],
    ...
    if (debug_ && tk2calo[itk] == -1) {
      ...
      if (ibest != -1)
        dbgPrintf(
            "%s","PFAlgo3 \t track %3d (pt %7.2f) would match to calo %3d (pt %7.2f) with dr %.3f if the pt min and dr "
...
void PFAlgo3::unlinkedtk_algo(Region &r, const std::vector<int> &tk2calo) const {
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (tk.floatPt() < maxPt) {
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) not matched to calo, kept as charged hadron\n", itk, tk.floatPt());
      auto &p = addTrackToPF(r, tk);
      p.hwStatus = GoodTK_NoCalo;
      tk.used = true;
    } else {
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) not matched to calo, dropped\n", itk, tk.floatPt());
...
void PFAlgo3::calo_relink(Region &r,
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    ...
    for (int ic2 = 0; ic2 < nc; ++ic2) {
      ...
      //if (debug_ && dr < 0.5) dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f) with no tracks is at dr %.3f from calo %3d with pt %7.2f (sum tk pt %7.2f), track excess %7.2f +- %7.2f\n", ic, calo.floatPt(), dr, ic2, calo2.floatPt(), calo2sumtkpt[ic2], calo2sumtkpt[ic2] - calo2.floatPt(), useTrackCaloSigma_ ? calo2sumtkpterr[ic2] : calo2.floatPtErr());
      if (dr < drbest) {
        float ptdiff =
    ...
    if (i2best != -1) {
      ...
      if (debug_)
        dbgPrintf(
            "%s","PFAlgo3 \t calo  %3d (pt %7.2f) with no tracks matched within dr %.3f with calo %3d with pt %7.2f (sum tk "
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    if (addtopt[ic]) {
      auto &calo = r.calo[ic];
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f, sum tk pt %7.2f) is increased to pt %7.2f after merging\n",
                  ic,
                  calo.floatPt(),
                  calo2sumtkpt[ic],
...
void PFAlgo3::linkedcalo_algo(Region &r,
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    ...
    if (debug_)
      dbgPrintf(
          "%s","PFAlgo3 \t calo  %3d (pt %7.2f +- %7.2f, empt %7.2f) has %2d tracks (sumpt %7.2f, sumpterr %7.2f), ptdif "
          "%7.2f +- %7.2f\n",
          ic,
          calo.floatPt(),
          calo.floatPtErr(),
    ...
    if (ptdiff > +ptMatchHigh_ * pterr) {
      if (ecalPriority_) {
        if (calo.floatEmPt() > 1) {
          ...
          if (debug_)
            dbgPrintf(
                "%s","PFAlgo3 \t calo  %3d (pt %7.2f, empt %7.2f)    ---> make photon with pt %7.2f, reduce ptdiff to %7.2f "
                "+- %7.2f\n",
                ic,
                calo.floatPt(),
                calo.floatEmPt(),
        ...
        if (ptdiff > 2) {
          if (debug_)
            dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f, empt %7.2f)    ---> make also neutral hadron with pt %7.2f\n",
                      ic,
                      calo.floatPt(),
                      calo.floatEmPt(),
                      ptdiff);
        ...
        if (debug_)
          dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f)    ---> promoted to neutral with pt %7.2f\n",
                    ic,
                    calo.floatPt(),
                    ptdiff);
        auto &p = addCaloToPF(r, calo);
      ...
      if (debug_)
        dbgPrintf(
            "%s","PFAlgo3 \t calo  %3d (pt %7.2f)    ---> to be deleted, will use tracks instead\n", ic, calo.floatPt());
      ...
      if (debug_ && rescaleTracks_)
        dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f)    ---> tracks overshoot and will be scaled down by %.4f\n",
                  ic,
                  calo.floatPt(),
                  calo2alpha[ic]);
      if (debug_ && !rescaleTracks_)
        dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f)    ---> tracks overshoot by %.4f\n",
                  ic,
                  calo.floatPt(),
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (calo.hwFlags == 1) {
      ...
      if (calo2ntk[tk2calo[itk]] == 1 && caloTrkWeightedAverage_) {
        ...
        if (debug_)
          dbgPrintf(
              "%s","PFAlgo3 \t track %3d (pt %7.2f +- %7.2f) combined with calo %3d (pt %7.2f +- %7.2f (from tk) yielding "
        ...
        p.hwStatus = GoodTK_Calo_TkPt;
        if (debug_)
          dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) linked to calo %3d promoted to charged hadron\n",
                    itk,
                    tk.floatPt(),
                    tk2calo[itk]);
      }
      ...
      if (debug_)
        dbgPrintf(
            "%s","PFAlgo3 \t track %3d (pt %7.2f, stubs %2d chi2 %7.1f) linked to calo %3d promoted to charged hadron with "
...
void PFAlgo3::unlinkedcalo_algo(Region &r) const {
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    if (!r.calo[ic].used) {
      ...
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t calo  %3d (pt %7.2f) not linked, promoted to neutral\n", ic, r.calo[ic].floatPt());
    }
  }
}
...
void PFAlgo3::save_muons(Region &r, const std::vector<int> &tk2mu) const {
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    if (r.track[itk].muonLink) {
      ...
      if (debug_)
        dbgPrintf("%s","PFAlgo3 \t track %3d (pt %7.2f) promoted to muon.\n", itk, r.track[itk].floatPt());
    }
  }
}
