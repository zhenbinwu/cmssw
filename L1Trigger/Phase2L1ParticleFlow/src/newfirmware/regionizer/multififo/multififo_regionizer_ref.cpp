...
void l1ct::MultififoRegionizerEmulator::setEgInterceptMode(bool afterFifo, const l1ct::EGInputSelectorEmuConfig & interceptorConfig) {
...
bool l1ct::MultififoRegionizerEmulator::step(bool newEvent,
  ...
  return nem_ ? emCaloRegionizer_.step(newEvent, links, out, mux) : 0;
}
...
{
  bool ret = false; 
  if (ntk_) ret = tkRegionizer_.step(newEvent, links_tk, out_tk, mux); 
  if (nmu_) ret = muRegionizer_.step(newEvent, links_mu, out_mu, mux); 
  switch (emInterceptMode_) {
      case noIntercept:
          if (ncalo_) ret = hadCaloRegionizer_.step(newEvent, links_hadCalo, out_hadCalo, mux); 
          if (nem_) ret = emCaloRegionizer_.step(newEvent, links_emCalo, out_emCalo, mux); 
          break;
