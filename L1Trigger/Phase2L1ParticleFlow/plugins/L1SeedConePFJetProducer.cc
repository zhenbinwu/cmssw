...
private:
  ...
  l1t::PFJet makeJet_HW(const std::vector<edm::Ptr<l1t::PFCandidate>>& parts) const;
};
...
l1t::PFJet L1SeedConePFJetProducer::makeJet_HW(const std::vector<edm::Ptr<l1t::PFCandidate>>& parts) const {
  ...
  auto sumpt = [](pt_t(a), const edm::Ptr<l1t::PFCandidate>& b) { return a + (pt_t)b->pt(); };
  ...
  std::transform(parts.begin(), parts.end(), pt_deta.begin(), [&seed, &pt](const edm::Ptr<l1t::PFCandidate>& part) {
    ...
    // Scale the eta, coordinate to the HW system
    return pt_etaphi_t(pt_t(part->pt()) *
                       detaphi_t(etaphi_t(part->eta() * etaphi_base) - etaphi_t(seed->eta() * etaphi_base)));
  });
