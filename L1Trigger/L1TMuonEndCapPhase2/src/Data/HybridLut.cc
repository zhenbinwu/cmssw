...
HybridLut::HybridLut() {
  ...
  etaRegionId_[2] = 4;
  etaRegionId_[3] = 3;
  etaRegionId_[4] = 5; // inner of ME11
...
const int& HybridLut::lookup_tflayer(const int& ring, const int& station) const {
  ...
  std::cout << " ring " <<ring <<" station " << station << " found " << found->second << std::endl;
...
const int HybridLut::lookup_phi(const int& sector, const int& chamber) const {
  
  int locphi = HybridLut::lookup_secphi(chamber);

  int globphi = locphi + 6*(sector-1);
