...
void L1Analysis::L1AnalysisPhaseII::SetTkEGV2(const edm::Handle<l1t::TkElectronCollection> tkElectronV2,
                                              const edm::Handle<l1t::TkElectronCollection> tkElectronV2HGC,
                                              unsigned maxL1Extra) {
  for (l1t::TkElectronCollection::const_iterator it = tkElectronV2->begin();
       it != tkElectronV2->end() && l1extra_.nTkElectronsV2 < maxL1Extra;
       it++) {
    if (it->et() > 5) {
  ...
  for (l1t::TkElectronCollection::const_iterator it = tkElectronV2HGC->begin();
       it != tkElectronV2HGC->end() && l1extra_.nTkElectronsV2 < maxL1Extra;
       it++) {
    if (it->et() > 5) {
      l1extra_.tkElectronV2Et.push_back(it->et());
      l1extra_.tkElectronV2Eta.push_back(it->eta());
