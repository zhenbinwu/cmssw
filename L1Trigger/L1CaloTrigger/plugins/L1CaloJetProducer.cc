...
void L1CaloJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    for (auto& hit : *l1CaloTowerHandle.product())
    {
        ...
        if (l1Hit.ecalTowerEt < EcalTpEtMin) l1Hit.ecalTowerEt = 0.0;
        if (l1Hit.hcalTowerEt < HcalTpEtMin) l1Hit.hcalTowerEt = 0.0;
        ...
        if (debug) printf("Tower iEta %i iPhi %i eta %f phi %f ecal_et %f hcal_et %f total_et %f\n", (int)l1Hit.towerIEta, (int)l1Hit.towerIPhi, l1Hit.towerEta, l1Hit.towerPhi, l1Hit.ecalTowerEt, l1Hit.hcalTowerEt, l1Hit.total_tower_et);
    }
    ...
    while (!caloJetClusteringFinished && n_towers != n_stale)
    {
        ...
        for (auto &l1CaloTower : l1CaloTowers)
        {
            ...
            if (caloJetObj.jetClusterET == 0.0) // this is the first l1CaloTower to seed the jet
            {
                ...
                reco::Candidate::PolarLorentzVector hcalP4( l1CaloTower.hcalTowerEt, l1CaloTower.towerEta, l1CaloTower.towerPhi, 0.);
                reco::Candidate::PolarLorentzVector ecalP4( l1CaloTower.ecalTowerEt, l1CaloTower.towerEta, l1CaloTower.towerPhi, 0.);
                reco::Candidate::PolarLorentzVector l1egP4( l1CaloTower.l1egTowerEt, l1CaloTower.towerEta, l1CaloTower.towerPhi, 0.);
                reco::Candidate::PolarLorentzVector totalP4( l1CaloTower.total_tower_et, l1CaloTower.towerEta, l1CaloTower.towerPhi, 0.);
                ...
                if (l1egP4.pt() > 0) 
                {
                    ...
                    if (l1CaloTower.isBarrel)
                    {
                        ...
                        std::vector< float > l1EG_info = {float(l1egP4.pt()), float(hcalP4.pt()), float(ecalP4.pt()), 0., 0., float(l1CaloTower.l1egTrkSS),
                            float(l1CaloTower.l1egTrkIso), float(l1CaloTower.l1egStandaloneSS), float(l1CaloTower.l1egStandaloneIso)};
                ...
                if (debug) printf(" -- hit %i, seeding input     p4 pt %f eta %f phi %f\n", cnt, l1CaloTower.total_tower_et, l1CaloTower.towerEta, l1CaloTower.towerPhi);
                    ...
                    ( fabs( d_eta ) < 0.13 && fabs( d_phi ) < 0.22 ) )
                {
                    ...
                    if (l1egP4.pt() > 0) 
                    {
                        ...
                        std::vector< float > l1EG_info = {float(l1egP4.pt()), float(hcalP4.pt()), float(ecalP4.pt()),
                            float(d_iEta), float(d_iPhi), float(l1CaloTower.l1egTrkSS), float(l1CaloTower.l1egTrkIso),
                            float(l1CaloTower.l1egStandaloneSS), float(l1CaloTower.l1egStandaloneIso)};
                        if (l1EG_info[1] / (l1EG_info[0] + l1EG_info[2]) < 0.25)
                        {
                            caloJetObj.n_l1eg_HoverE_LessThreshold++;
                        }
