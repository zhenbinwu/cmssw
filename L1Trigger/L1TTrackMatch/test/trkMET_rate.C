...
void trkMET_rate() {
  ...
  float numBins = 100.0; float cutoff = 200.0-0.1;
  float TP_minPt = 2.0; float TP_maxEta = 2.4;
  float chi2dof_cut = 10.0; float bendchi2_cut = 2.2;

  TH1D *trueTkMET_thresh = new TH1D("trueTkMET_thresh","trueTkMET_thresh;trueTkMET threshold (GeV);Occurrences", numBins, 0,numBins);//using tracking particles
  TH1D *recoTkMET_thresh = new TH1D("recoTkMET_thresh","recoTkMET_thresh;recoTkMET threshold (GeV);Occurrences", numBins, 0,numBins);//using tracks
  ...
  for (int i=0; i<nevt; i++) {
    tree->GetEntry(i);
    if (i % 10000 == 0) std::cout << i << "/" << nevt << std::endl;
    ...
    if (rerun_trueTkMET) {
      ...
      for (int it=0; it<(int)tp_pt->size(); it++) {
        ...
        if (fabs(this_tp_eta) > TP_maxEta) continue;
        if (tp_nstub->at(it) < 4) continue;
        if (fabs(tp_z0->at(it))>15) continue;
        // if (this_tp_signal!=0) continue;
        // if (tp_dxy->at(it) > 1) continue;
        if (tp_charge->at(it)==0) continue;

        float deltaZ_cut = 3.0; // cuts out PU
        if      ( fabs(this_tp_eta)>=0   &&  fabs(this_tp_eta)< 0.7) deltaZ_cut = 0.4;
        else if ( fabs(this_tp_eta)>=0.7 &&  fabs(this_tp_eta)< 1.0) deltaZ_cut = 0.6;
        else if ( fabs(this_tp_eta)>=1.0 &&  fabs(this_tp_eta)< 1.2) deltaZ_cut = 0.76;
        else if ( fabs(this_tp_eta)>=1.2 &&  fabs(this_tp_eta)< 1.6) deltaZ_cut = 1.0;
        else if ( fabs(this_tp_eta)>=1.6 &&  fabs(this_tp_eta)< 2.0) deltaZ_cut = 1.7;
        else if ( fabs(this_tp_eta)>=2.0 &&  fabs(this_tp_eta)<=2.4) deltaZ_cut = 2.20;
    ...
    if (rerun_recoTkMET) { //also useful for checking different track quality cuts
      float recoTkMET_calc = 0; float recoTkMETx = 0; float recoTkMETy = 0;
      ...
      for (int it=0; it<(int)trk_pt->size(); it++) {
        ...
        float deltaZ_cut = 3.0; // cuts out PU
        if      ( fabs(thisTrk_eta)>=0   &&  fabs(thisTrk_eta)< 0.7) deltaZ_cut = 0.4;
        else if ( fabs(thisTrk_eta)>=0.7 &&  fabs(thisTrk_eta)< 1.0) deltaZ_cut = 0.6;
        else if ( fabs(thisTrk_eta)>=1.0 &&  fabs(thisTrk_eta)< 1.2) deltaZ_cut = 0.76;
        else if ( fabs(thisTrk_eta)>=1.2 &&  fabs(thisTrk_eta)< 1.6) deltaZ_cut = 1.0;
        else if ( fabs(thisTrk_eta)>=1.6 &&  fabs(thisTrk_eta)< 2.0) deltaZ_cut = 1.7;
        else if ( fabs(thisTrk_eta)>=2.0 &&  fabs(thisTrk_eta)<=2.4) deltaZ_cut = 2.20;

        if (thisTrk_pt<TP_minPt || fabs(thisTrk_eta) > TP_maxEta || thisTrk_nstub<4 || fabs(thisTrk_z0)>15) continue;
  			if (fabs(deltaZ)>deltaZ_cut) continue;
  ...
  // -------------------------------------------------------------------------------------------
  trueTkMET_thresh->SetStats(0); recoTkMET_thresh->SetStats(0);
  trueTkMET_thresh->SetLineColor(kBlack); recoTkMET_thresh->SetLineColor(kRed);
  trueTkMET_thresh->SetTitle(""); recoTkMET_thresh->SetTitle("");
  ...
  fCumulative_true->SetMarkerSize(0.7); fCumulative_true->SetMarkerStyle(20);
	fCumulative_true->SetMarkerColor( fCumulative_true->GetLineColor() );

	fCumulative_reco->SetMarkerSize(0.7); fCumulative_reco->SetMarkerStyle(20);
  ...
  std::cout<<"Track MET Threshold at 35kHz "<<fCumulative_reco->GetBinLowEdge(fCumulative_reco->FindLastBinAbove(35))<<std::endl;
