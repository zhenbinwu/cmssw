...
void trkMET_turnOn() {
  ...
  float numBins = 100.0; float cutoff = 300.0-0.1;
  ...
  for (int i=0; i<nevt; i++) {
    ...
    if (rerun_trueTkMET) {
      ...
      for (int it=0; it<(int)tp_pt->size(); it++) {
        ...
        if (this_tp_eventID!=0) continue; // hard interaction only
    ...
    if (rerun_recoTkMET) { //also useful for checking performance of new track quality cuts
      ...
      if (recoTkMET_calc!=recoTkMET_ntuple) std::cout<<"Ugh: "<< recoTkMET_calc <<", "<<recoTkMET_ntuple<<std::endl;
    ...
    if (trueTkMET_ntuple>49)  h_trueTkMET_num->Fill(trueMET_ntuple);
    if (recoTkMET_ntuple>49) h_recoTkMET_num->Fill(trueMET_ntuple);
  } // end event loop


  //Draw
  h_trueMET->SetLineColor(kBlack); h_trueMET->SetLineStyle(2);
  ...
  h_trueMET->Rebin(2); h_trueTkMET_num->Rebin(2); h_recoTkMET_num->Rebin(2);
  h_trueMET->Sumw2(); h_trueTkMET->Sumw2(); h_trueTkMET_num->Sumw2();
  ...
  h_TkMET_turnon->SetMarkerStyle(20); h_TkMET_turnon->SetMarkerSize(0.7);
  h_TkMET_turnon->SetMarkerColor(kBlack); h_TkMET_turnon->SetLineColor(kBlack);
  ...
  h_recoTkMET_turnon->SetMarkerStyle(20); h_recoTkMET_turnon->SetMarkerSize(0.7);
  h_recoTkMET_turnon->SetMarkerColor(kRed); h_recoTkMET_turnon->SetLineColor(kRed);
  ...
  std::cout<<"Online (true) Track MET at "<<trueTkMET_rate<<" GeV = "<<trueTkMET_95eff<<" GeV (offline)"<<std::endl;
  std::cout<<"Online (reco) Track MET at "<<recoTkMET_rate<<" GeV = "<<recoTkMET_95eff<<" GeV (offline)"<<std::endl;
