...
void METTrackTransform::GenerateLUTs(){
...
InternalEtWord METTrackTransform::TransformTrack(TTTrack< Ref_Phase2TrackerDigi_ >& track_ref, float PV){
    InternalEtWord Outword;

    Outword.pV = digitize_Signed <iZ0> (PV, -maxmap["z0"], maxmap["z0"], binmap["z0"]); //Convert vertex TODO use emulator
    ...
    Outword.chi2rzdof   = Chi_to_FWChi <ichi2XY> (track_ref.chi2Z(),chi2Values,binmap["chixy"]); //TTTrack_TrackWord has no chi2Z getter function 
...
InternalEtWord METTrackTransform::TransfromCuts(const edm::ParameterSet& iConfig){
...
iglobPhi METTrackTransform::local_to_global(iPhi local_phi,iglobPhi sector_shift){ 
...
iNstub METTrackTransform::CountNStub(unsigned int Hitpattern){  
    iNstub Nstub = 0;
...
iPhi METTrackTransform::FloatPhi_to_iPhi(float phi,unsigned int sector){   
    float temp_phi = 0.0;
    if (sector < 4 ){ 
...
std::vector<iglobPhi> METTrackTransform::generate_phi_slice_LUTs(unsigned int N) {
...
chi METTrackTransform::Chi_to_FWChi(float Chi2,const float bins[],unsigned int N){  
    chi outputChi = 0;
