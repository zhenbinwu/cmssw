...
namespace Phase2L1GMT
{
    ...
    for (unsigned int i = 0; i < 12; ++i)
    {
      if (i < trkMus->size())
        dumpInput <<" " << nevti <<" 0 " << i <<" "<<trkMus->at(i).hwPt() * lsb_pt<<" " <<trkMus->at(i).hwEta() * lsb_eta
          <<" " << trkMus->at(i).hwPhi() * lsb_phi <<" " << trkMus->at(i).hwZ0() * lsb_z0 <<" " <<trkMus->at(i).charge() << std::endl;
      else
        dumpInput<<" " << nevti << " 0 " << i <<" " <<0 <<" " <<0 <<" " <<0 <<" " <<0  <<" " <<0  <<std::endl;
      totalsize++;
    }
    for (unsigned int i = 0; i < convertedTracks->size(); ++i)
    {
      dumpInput <<" " << nevti <<" 1 "<< i <<" "<<convertedTracks->at(i).pt() * lsb_pt 
        << " " <<convertedTracks->at(i).eta() * lsb_eta
        <<" " << convertedTracks->at(i).phi() * lsb_phi<<" " << convertedTracks->at(i).z0() *lsb_z0 <<" " 
        << convertedTracks->at(i).charge() <<" "<< convertedTracks->at(i).quality()<< std::endl;
      totalsize++;
    }
  ...
  int TopoAlgo::deltaPhi(int phi1, int phi2) {
    ...
    static const int pibits = (1 << (BITSPHI-1)) -1;
