...
struct HGCalSiliconRotatedCassette {
  ...
  void positionSensitive(cms::DDParsingContext& ctxt, xml_h e, const dd4hep::Volume& glog, int layer) {
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconRotatedCassette: PositionSensitive layer " << glog.name() << "  r " << cms::convert2mm(delx)
    ...
    for (int k = 0; k < cassettes_; ++k) {
      ...
      for (int n = 0; n < num; ++n) {
        ...
        edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconRotatedCassette::Passive2 " << passive << " number " << i2
                                      << " pos " << cms::convert2mm(xpos) << ":" << cms::convert2mm(ypos) << ":" << cms::convert2mm(zpos);
        kount++;
#endif
